...
#if !defined(DEBUG_SEMIHOSTING)
__attribute__ ((weak)) int WRITEFUNC(int iFileHandle, char *pcBuffer, int iLength)
{
#if defined(DEBUG_ENABLE)
	unsigned int i;
	for (i = 0; i < iLength; i++) {
...
__attribute__ ((weak)) int READFUNC(void)
{
#if defined(DEBUG_ENABLE)
 ...
 * of bytes received */
__attribute__ ((weak)) int READFUNC(int iFileHandle, char *pcBuffer, int iLength)
{
	int idx;
